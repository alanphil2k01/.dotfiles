...
local ICONS_DIR = HOME_DIR .. '/.config/awesome/theme/icons'
...
local function create_kill_process_button()
    return wibox.widget{
        {
            id = "icon",
            image = ICONS_DIR .. '/window-close-symbolic.svg',
            resize = false,
            opacity = 0.1,
...
local function worker(args)
    ...
    local cpus = {}
    watch([[bash -c "grep '^cpu.' /proc/stat; ps -eo '%p|%c|%C|' -o "%mem" -o '|%a' --sort=-%cpu | head -6 | tail -n +2"]], timeout,
                ...
                for line in stdout:gmatch("[^\r\n]+") do
                    if starts_with(line, 'cpu') then
                        ...
                        if is_update == true then
                            ...
                            pid_name_rest:ajust_ratio(2, 0.2, 0.4, 0.4)
